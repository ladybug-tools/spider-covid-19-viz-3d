...
PTS.getPoints = function (places, index = 2, color = "red", ) {
	...
	for (let i = 0; i < places.length; i++) {
		...
		if (find) {
			...
			let heightScaled = PTS.scale ? 0.05 * Math.sqrt( height ) : 0.001 * height;
			heightScaled = heightScaled < 1 ? 1 : heightScaled;
			console.log( "height", heightScaled );


			const matrix = getMatrixComposed({ radius: 50, latitude: + find[2], longitude: + find[3], height: heightScaled });
